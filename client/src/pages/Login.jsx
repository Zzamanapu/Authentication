...
const Login = () => {
  ...
  const onSubmitHandler = async (e) => {
    try {
      ...
      axios.defaults.withCredentials = true //send cookie to backed with the data
    ...
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-linear-to-br from-blue-200 to-purple-400'>
      ...
      <div className='bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm'>
        ...
        <form onSubmit={onSubmitHandler}>
          {state === 'Sign Up' && (
            <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A53]'>
          ...
          <button className='w-full py-2.5 rounded-full bg-linear-to-r from-indigo-500 to bg-indigo-900 text-white font-medium cursor-pointer'>{state}</button>
        </form>
